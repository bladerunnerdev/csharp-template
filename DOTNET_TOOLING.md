...
- [.NET tooling notes: Visual Studio vs. VS Code vs. CLI](#net-tooling-notes-visual-studio-vs-vs-code-vs-cli)
  - [Adding different project types](#adding-different-project-types)
    - [Which type for Domain / Infrastructure / Data / Application layers?](#which-type-for-domain--infrastructure--data--application-layers)
  - [Adding new files: classes, interfaces, and other "Add New Item" equivalents](#adding-new-files-classes-interfaces-and-other-add-new-item-equivalents)
    - [Plain C# types (class, interface, enum, struct, record) — no CLI equivalent exists](#plain-c-types-class-interface-enum-struct-record--no-cli-equivalent-exists)
    - [Item types `dotnet new` does support](#item-types-dotnet-new-does-support)
    - [Config and misc files](#config-and-misc-files)
  - [Overriding built-in code snippets (`class`, `ctor`, etc.)](#overriding-built-in-code-snippets-class-ctor-etc)
    - [Option A: add your own snippet with the same prefix](#option-a-add-your-own-snippet-with-the-same-prefix)
    - [Option B (more durable): turn on the accessibility-modifier analyzer](#option-b-more-durable-turn-on-the-accessibility-modifier-analyzer)
  - [XML documentation comments (the JSDoc equivalent)](#xml-documentation-comments-the-jsdoc-equivalent)
  - [Scaffolding with `dotnet-aspnet-codegenerator`](#scaffolding-with-dotnet-aspnet-codegenerator)
    - [Install](#install)
    - [Generators and common commands](#generators-and-common-commands)
  - [Where they genuinely differ](#where-they-genuinely-differ)
